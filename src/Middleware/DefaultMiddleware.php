...
class DefaultMiddleware implements MiddlewareInterface
{
    ...
    private function handler(PsrRequestInterface $request): PsrResponseInterface
    {
        ...
        list($status, $className) = \bean('grpcServerRouter')->match($router);
        list($object, $method) = $this->getReflectionClassAndMethod($router, $className);

        if (!method_exists($object, $method)) {
            throw new GrpcServerException(sprintf('Method(%s::%s) 不存在', PsrRequestInterface::class, $method), 404);
        }
                    ...
                    case 'object':
                        ...
                        if (!BeanFactory::getBean($definition['ref']) && !$definition['allowsNull']) {
                            throw new GrpcServerException(
                                sprintf('grpcError: Argument %s invalid, object %s not found.', $definition['name'], $definition['ref'])
                            );
                        }
